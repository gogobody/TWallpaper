...
class JiangQie_API_Setting_Controller extends JiangQie_API_Base_Controller
{
	...
	public function register_routes()
	{
		...
		//分享配置
		register_rest_route($this->namespace, '/' . $this->module . '/share', [
			[
		...
		//热门
		register_rest_route($this->namespace, '/' . $this->module . '/advertisement', [
			[
				'callback' => [$this, 'get_advertisement'],
			]
		]);
	...
	{
			//banner
			$data['banner'] = JiangQie_API::option_value('bannerid');
			//插屏广告
			$data['interstitialid'] = JiangQie_API::option_value('interstitialid');
			//视频广告
			$data['videoid'] = JiangQie_API::option_value('videoid');
			//格子广告
			$data['gridid'] = JiangQie_API::option_value('gridid');
			//激励视频广告
			$data['rewardedVideoid'] = JiangQie_API::option_value('rewardedVideoid');
			//原生广告A
			$data['customAid'] = JiangQie_API::option_value('customAid');
			//原生广告B
			$data['customBid'] = JiangQie_API::option_value('customBid');
			// 广告位置信息
			$data['posisionad'] = JiangQie_API::option_value('posisionad');
			// 无限插屏
			$data['switch_inad'] = JiangQie_API::option_value('switch_inad');
			return $this->make_success($data);
	}
// 获取流量主配置
	...
	{
		//分享标题
		$data['share_title'] = JiangQie_API::option_value('share_title');
		//分享封面
		$data['share_image'] = JiangQie_API::option_value('share_image');
		return $this->make_success($data);
		}
	/**
	 * 获取配置 热门
