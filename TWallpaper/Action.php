...
class TWallpaper_Action extends Typecho_Widget implements Widget_Interface_Do
{
    ...
    public function __construct($request, $response, $params = NULL)
    {
        ...
        $this->plugin_dir = Helper::options()->pluginDir('TWallpaper') . '/TWallpaper';
//        var_dump($this->request->getRequestUrl());
    ...
    {
        $options = Helper::options()->plugin('TWallpaper');
    ...
    public function get_post_meta($post_id, $key, $single = false)
    {
        if ($key == 'views') {
            ...
            return $this->db->fetchObject($this->db->select('cid','name','str_value')->from('table.fields')->where('cid = ? AND name = ?', $post_id, $key))->str_value;
        }
    }
...
//                }
//            }
            $item['thumbnail'] = self::GetRandomThumbnail($post['text'], self::get_post_meta($post['cid'], 'thumb'));
            $data[] = $item;
        }
        return $data;
    }
    ...
    {
        $select = $select->from('table.contents')->where('table.contents.type = ? and table.contents.status = ?', 'post', 'publish');
        if (isset($args['s']) and !empty($args['s'][0])) { //search
            ...
            $select = $select->join('table.relationships', 'table.contents.cid = table.relationships.cid', Typecho_Db::LEFT_JOIN)
                ...
                ->where('table.contents.title LIKE ? OR table.contents.text LIKE ? OR table.metas.name LIKE ?', $search_param, $search_param,$search_param);
        }elseif (isset($args['category__not_in']) and !empty($args['category__not_in'][0])) {
            $select = $select->join('table.relationships', 'table.contents.cid = table.relationships.cid', Typecho_Db::LEFT_JOIN)
                ->join('table.metas', 'table.metas.mid = table.relationships.mid', Typecho_Db::LEFT_JOIN)->where('table.metas.type = ?','category');

            foreach ($args['category__not_in'] as $ext) {
                $select = $select->where('table.relationships.mid <> ?', $ext);
            }
        }
        ...
        if (isset($args['post__not_in']) and !empty($args['post__not_in'][0])) {
            foreach ($args['post__not_in'] as $ext) {
                $select = $select->where('table.contents.cid <> ?', $ext);
            }
//            $select = $select->where('cid NOT IN ?',$args['post__not_in']);
        }
        if (isset($args['post__in']) and !empty($args['post__in'][0])) {
            $select = $select->where('table.contents.cid IN ?', $args['post__in']);
        }
...
//        }
//        return $posts;
        return $this->db->fetchAll($this->db->select('table.contents.*')->from('table.contents')->where('cid IN ?', $sticky_posts));
    }
    ...
    public function get_posts($args, $qstick = false)
    {
        if ($qstick && self::option_value('JSwitch_stick') == '1') { // 置顶
            ...
            $select = $this->db->select('table.contents.*');
        ...
        foreach ($posts as &$post) {
            //查询t category
            $category = $this->get_the_category($post['id']);
            if (!$category) {
                $post['category'] = [];
            } else {
                $post['category'] = [
...
//            $post['views'] = self::get_post_meta($post['id'], 'views', true);
    ...
    public static function GetRandomThumbnail($content, $thumb = "")
    {
        ...
        $options = Typecho_Widget::widget('Widget_Options')->plugin('TWallpaper');
    ...
    public function get_home()
    {
        ...
        if (!empty($slide_ids)) {
            ...
            foreach ($result as $item) {
                $slides[] = [
                    'id' => $item['cid'],
                    'thumbnail' => self::GetRandomThumbnail($item['text'], self::get_post_meta($item['cid'], 'thumb'))
                ];
            }
        }
        $data['slide'] = $slides;
...
//        }
        ...
        if (!empty($hot_ids)) {
            ...
            foreach ($result as $item) {
                $category_ = self::get_the_category($item['cid']);
                $hots[] = [
                    'id' => $item['cid'],
                    'title' => $item['title'],
                    'category' => empty($category_[0]) ? '' : $category_[0],
                    'thumbnail' => self::GetRandomThumbnail($item['text'], self::get_post_meta($item['cid'], 'thumb'))
                ];
            }
        }
...
//        }
    ...
    {
        $data['enableNoAd'] = self::option_value('JEnable_noad_download') == '1';
        //banner
        $data['banner'] = self::option_value('JBannerid');
        //插屏广告
        $data['interstitialid'] = self::option_value('JInterstitialid');
        //视频广告
        $data['videoid'] = self::option_value('JVideoid');
        //格子广告
        $data['gridid'] = self::option_value('JGridid');
        //激励视频广告
        $data['rewardedVideoid'] = self::option_value('JRewardedVideoid');
        //原生广告A
        $data['customAid'] = self::option_value('JCustomAid');
        //原生广告B
        $data['customBid'] = self::option_value('JCustomBid');
        // 广告位置信息
        $data['posisionad'] = self::option_value('JPosisionad') ? self::option_value('JPosisionad') : [];
        // 无限插屏
        $data['switch_inad'] = self::option_value('JSwitch_inad');
        return $this->make_success($data);
    }
    ...
    {
        //分享标题
        $data['share_title'] = self::option_value('JShare_title');
        //分享封面
        $data['share_image'] = self::option_value('JShare_image');
        return $this->make_success($data);
    }
...
//        $this->db->query($this->db->delete('table.contents')->where('cid = ?',$post_id));
    ...
    public function send_post($url, $post_data,$string = false) {
        if ($string){
    ...
    public function msg_sec_check($content)
    {
        ...
        $args = json_encode(array('content'=>$content),JSON_UNESCAPED_UNICODE);

        $res = $this->send_post($api, $args);
        $res = json_decode($res);
        if ($res->errcode == 0) {
            return true;
        }
...
//        $this->widget('Widget_Archive@'.$post_id, 'pageSize=1&type=post', 'cid='.$post_id)->to($postObj);
        $postObj = $this->db->fetchObject($this->db->select('table.contents.*')->from('table.contents')->where('cid = ?', $post_id));
        $post = [
            'id' => $postObj->cid,
            ...
            'thumbnail' => self::GetRandomThumbnail($postObj->text, self::get_post_meta($postObj->cid, 'thumb'))
        ];
...
//        $this->widget('Widget_Archive@'.$page_id, 'pageSize=1&type=post', 'cid='.$page_id)->to($post);
        $post = $this->db->fetchObject($this->db->select('table.contents.*')->from('table.contents')->where('cid = ?', $page_id));
...
//        }
        ...
        foreach ($posts as &$post) {
            //查询 category
            $category = self::get_the_category($post['id']);
            if (!$category) {
                $post['category'] = [];
            } else {
                $post['category'] = [
    ...
    public function get_wxacode()
    {
        ...
        // 小程序码数量有限，所以这里改成固定的
        $qrname = 'wxacode-' . $post_type . '-' . 'qrcode' . '.png';
        $qrcode = $qrcode_path . $qrname;
        $qrcode_link = $uploads['url'] . '/wxacode/' . $qrname;
        if (is_file($qrcode)) {
